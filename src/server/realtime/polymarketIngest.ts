...
type Side = 'YES' | 'NO'
type MarketSides = { yesAsset: string; noAsset: string }
...
const POLY_WS_URL =
  process.env.POLY_WS_URL || 'wss://ws-subscriptions-clob.polymarket.com/ws/market'

const PING_INTERVAL_MS = Number(process.env.POLY_WS_PING_INTERVAL_MS || 10_000)
...
function normalizePrice(x: any): number | null {
  const n = Number(x)
 ...
 */
function extractPriceFromMsg(msg: any): number | null {
 ...
 */
function extractAssetId(msg: any): string | null {
  if (!msg || typeof msg !== 'object') return null
  const data = msg.data ?? msg.payload ?? msg
  const asset =
    data.asset_id ??
 ...
 */
async function buildMappings(): Promise<{
  assetIds: string[]
  assetToMarket: Map<string, { marketId: string; side: Side }>
  marketToSides: Map<string, MarketSides>
}> {
  ...
  const marketToSides = new Map<string, MarketSides>()
...
async function mergeAndWritePrice(marketId: string, patch: Partial<PriceState>) {
  const key = redisKey(marketId)

  // Merge with existing so YES/NO can update independently.
  let existing: PriceState = { yes: null, no: null, updatedAt: 0 }
  ...
  if (next.yes == null && next.no == null) return
...
export function startPolymarketIngest() {
  ...
  ;(async () => {
    ...
    while (!stopped) {
      attempt += 1
      const backoff = Math.min(30_000, 1000 * Math.pow(1.6, attempt))
      ...
      try {
        ...
        ws.on('message', async (buf) => {
          ...
          for (const m of messages) {
            ...
            if (price == null || !isProb(price)) continue
