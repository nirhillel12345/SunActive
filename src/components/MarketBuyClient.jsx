...
export default function MarketBuyClient({ market, user }) {
    ...
    async function confirmBuy() {
        ...
        try {
            if (!selectedPrice)
                return toast.push({ title: 'Price unavailable', type: 'error' });
    ...
    useEffect(() => {
        let ws = null;
        let closed = false;
        const port = process.env.NEXT_PUBLIC_WS_PORT || (window.location.port || '4001');
    ...
    return (<Card className="p-4">
      ...
      <div className="mb-3">
        ...
        <input type="number" min={1} value={amount} onChange={(e) => setAmount(e.target.value === '' ? '' : Number(e.target.value))} className="w-full border rounded px-3 py-2"/>
      </div>
