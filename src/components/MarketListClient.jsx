...
export default function MarketListClient({ initialMarkets }) {
    ...
    const categories = useMemo(() => {
        const set = new Set();
        initialMarkets.forEach((m) => { if (m.category)
            set.add(m.category); });
        return Array.from(set);
    }, [initialMarkets]);
    const filtered = useMemo(() => {
        return initialMarkets.filter((m) => {
            if (openOnly && m.resolved)
                return false;
            if (category && m.category !== category)
                return false;
            if (query && !m.question.toLowerCase().includes(query.toLowerCase()))
                return false;
            return true;
        });
    ...
    return (<div>
      ...
      {loading ? (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 w-full">
          {filtered.map((m) => (<MarketCard key={m.id} market={{ id: m.id, title: m.question, question: m.question, category: m.category, resolved: m.resolved, volume: m.volume ?? 0, liquidity: m.liquidity ?? 0, probability: Math.round((m.liquidity ?? 0) % 100) }}/>))}
        </div>)}
    </div>);
}
