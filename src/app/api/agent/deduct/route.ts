...
import { agentDeductFromUser } from '@/server/services/agentService'
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const res = await agentDeductFromUser(agentId, data.userId, data.amount, data.note)
    return NextResponse.json({ success: true, data: res })
  } catch (err: any) {
    return NextResponse.json({ success: false, error: err?.message || 'Agent deduct failed' }, { status: 400 })
  }
}
