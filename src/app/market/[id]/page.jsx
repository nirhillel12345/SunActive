...
export default async function MarketPage({ params }) {
    ...
    let user = null;
    if (session?.user?.id) {
        user = await prisma.user.findUnique({ where: { id: String(session.user.id) }, select: { id: true, username: true, balancePoints: true } });
    }
    return (<div className="p-4 max-w-3xl w-full mx-auto">
